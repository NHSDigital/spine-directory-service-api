...
def build_endpoint_resources(ldap_attributes: Dict) -> List[Dict]:
    ldap_attributes = ldap_attributes.copy()
    for key in list(ldap_attributes.keys()):
        ldap_attributes[key.lower()] = ldap_attributes[key]
        del ldap_attributes[key]
    ...
    def build_endpoint(address):
        ...
        interaction_id = ldap_attributes.get("nhsMhsSvcIA".lower())
        if interaction_id:
        ...
        return result
    return [build_endpoint(address) for address in ldap_attributes['nhsMHSEndPoint'.lower()]]
...
def build_device_resource(ldap_attributes: Dict) -> Dict:
    ...
    extension = []
    manufacturing_organization = ldap_attributes.get('nhsIdCode'.lower())
    ...
    service_id_extensions = list(map(
        ...
        ldap_attributes.get('nhsAsSvcIA'.lower())))
...
def _build_extension_array(ldap_attributes: Dict):
    return [
        _build_string_extension("nhsMHSSyncReplyMode", ldap_attributes.get("nhsMHSSyncReplyMode".lower())),
        _build_string_extension("nhsMHSRetryInterval", ldap_attributes.get("nhsMHSRetryInterval".lower())),
        _build_int_extension("nhsMHSRetries", ldap_attributes.get("nhsMHSRetries".lower())),
        _build_string_extension("nhsMHSPersistDuration", ldap_attributes.get("nhsMHSPersistDuration".lower())),
        _build_string_extension("nhsMHSDuplicateElimination", ldap_attributes.get("nhsMHSDuplicateElimination".lower())),
        _build_string_extension("nhsMHSAckRequested", ldap_attributes.get("nhsMHSAckRequested".lower()))
    ]
