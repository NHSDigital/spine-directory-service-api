...
def create_connection() -> ldap3.Connection:
    if config.get_config(sds_mock_connection_factory.LDAP_MOCK_DATA_URL_CONFIG_KEY, default=None):
        ...
        return build_real_sds_connection(ldap_url, use_tls)


def build_real_sds_connection(ldap_url: str, tls: bool = True) -> ldap3.Connection:
    """Initialise the real SDS connection object

    :param ldap_url: The URL to communicate with LDAP on.
    :param tls: A flag to indicate whether TLS should be enabled for the SDS connection.
    :return:
    """
    logger.info('Configuring connection to SDS using {url} {tls}', fparams={"url": ldap_url, "tls": tls})
    ...
    if tls:
        ...
        sds_connection = build_sds_connection_tls(ldap_address=ldap_url,
                                                  private_key=client_key,
                                                  local_cert=client_cert,
                                                  ca_certs=ca_certs)
    else:
        sds_connection = build_sds_connection(ldap_address=ldap_url)
