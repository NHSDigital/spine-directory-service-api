...
module "spine-directory" {
  source                   = "github.com/NHSDigital/api-platform-service-module"
  name                     = "spine-directory"
  path                     = "spine-directory"
  ...
  api_product_display_name = length(var.namespace) > 0 ? "spine-directory${var.namespace}" : "Spine Directory Service"
